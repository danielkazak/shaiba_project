...
angular.module('shaibaApp')
  .service('parse', ['$http', '$q', 'ngProgress', 'SharedData', function ($http, $q, ngProgress, SharedData) {
        ...
        self.getRandomNumber = function(num){
        ...
        self.getTable = function(tableName, update) {
            ...
            if (!update && self[tableName] !== null) {
                ...
                console.log('not from db ' + tableName);
            } else {
                console.log('yes from db ' + tableName);
                ngProgress.start();
        ...
        self.getRandom = function(table) {
            ...
            if(typeof table === 'string') {
                self.getTable(table, false)
                    .then(
                    function (response) {
                        deferred.resolve(response[self.getRandomNumber(response.length)]);
                    },
                    function (response) {
                        deferred.reject(response);
                    });
            } else {
                deferred.resolve(table[self.getRandomNumber(table.length)]);
            }
